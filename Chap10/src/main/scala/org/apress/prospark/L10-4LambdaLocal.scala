...
object LambdaApp {
  ...
  def main(args: Array[String]) {
    if (args.length != 16) {
      System.err.println(
        "Usage: LambdaApp <appname> <batchInterval> <hostname> <port> <projectid>"
          + " <zone> <cluster> <tableName> <columnFamilyName> <columnName> <checkpointDir>"
          + " <sessionLength> <serviceAccount> <localKeyFile> <bqDatasetId> <bqTableId>")
      System.exit(1)
    }
    val Seq(appName, batchInterval, hostname, port, projectId, zone, clusterId,
      ...
      serviceAccount, localKeyFile, bqDatasetId, bqTableId) = args.toSeq
    ...
    ratings.foreachRDD(rdd => {
      ...
      bqConf.set("google.cloud.auth.service.account.enable", "true")
      bqConf.set("google.cloud.auth.service.account.email", serviceAccount)
      bqConf.set("google.cloud.auth.service.account.keyfile", localKeyFile)
      val bqTableSchema =
