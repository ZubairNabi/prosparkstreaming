  ...
  def main(args: Array[String]) {
    ...
    val cdrStream = ssc.socketTextStream(hostname, port.toInt)
      ...
      .foreachRDD(rdd => {
        ...
        println("#########Corr########:" + cdrs.stat.corr("smsOutActivity", "callOutActivity"))
        println("#########Cov#########:" + cdrs.stat.cov("smsInActivity", "callInActivity"))
        cdrs.stat.crosstab("squareId", "countryCode").show()
        cdrs.stat.freqItems(Array("squareId", "countryCode"), 0.1).show()
        //cdrs.stat.crosstab("callOutActivity", "callInActivity")
      })
